...
//        u.ensureIndependence();
        ...
        TenZ x;
        math21_operator_tensor_set_size_cpu(x, d, paras.start, 0);
...
//        ts.log("ts");
    ...
    void test_tensor_submatrix_1() {
        MatR A(3, 3);
        A.letters();
        MatR B;
        VecN row_ind(2);
        VecN col_ind(1);
        row_ind = 3, 1; // todo: error when not in order
//        row_ind = 1, 3;
        col_ind = 0;
    ...
    void test_tensor_submatrix_2() {
        ...
        colIndex.setSize(2);
        colIndex = 2, 3;
        math21_op_submatrix_get(B, A, rowIndex, colIndex);
        A.log("A");
        B.log("B");
    }

    void test_tensor_submatrix() {
        test_tensor_submatrix_1();
        test_tensor_submatrix_2();
    }
...
//        math21_operator_tensor_f_shrink(A, B, index, f_mean);
    ...
    void test_tensor_to_string() {
        ...
        A = 'm', 'a', 't', 'h', '2', '1';
        std::string s;
        math21_operator_tensor_get_string(A, s);
...
// https://en.cppreference.com/w/cpp/language/lifetime
    void test_object_use() {
        ...
        object_generate().get<TenR>().log("o");

        // The lifetime of a temporary object may be extended by binding to a const lvalue reference or to an rvalue reference (since C++11), see reference initialization for details.
        {
            const Uobj &o = object_generate();
            o.log("o1");
        }
        {
            Uobj &&o = object_generate();
            o.clear();
            o.log("o2");
        }
    }

    void test_opencl_align() {
        VecR x;
        x.setDeviceType(2);
        x.setSize(20);
        x.letters();
        VecR y;
        math21_operator_share_vector_part_using_from_to(x, y, 16, 20);
//        math21_operator_share_vector_part_using_from_to(x, y, 17, 20);
        x.log("x");
        y.log("y");
        math21_opencl_test3();
    }
    ...
    void test_move_constructor() {
        ...
        MATH21_ASSERT(y.isSameSize(2, 1));
        x = y;
        ...
        v.push(std::move(x));
        MATH21_ASSERT(x.isEmpty());
        MATH21_ASSERT(v(2).isSameSize(2, 1));
#endif
    }
