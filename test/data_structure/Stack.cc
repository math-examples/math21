...
namespace math21 {
    ...
    namespace data_structure {
        ...
        void math21_data_structure_queue_test_1() {
            std::string text = "to be or not to - be - - that - - - is";
            std::istringstream io;
            io.str(text);
            std::string item;
        ...
        void math21_data_structure_queue_test_2() {
            Queue<MatR> queue;
            NumN i = 0;
            NumN n = 10000;
            while (i < n) {
            ...
            while (!queue.isEmpty()) {
                ++i;
                MatR x = queue.dequeue();
                if (i == n / 2) {
                    MatR y;
                    y.setSize(x.shape());
                    y = n / 2;
                    MATH21_PASS(math21_op_isEqual(x, y));
                }
            }
        }

        void math21_data_structure_queue_test() {
            math21_data_structure_queue_test_1();
            math21_data_structure_queue_test_2();
        }
