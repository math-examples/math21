...
void math21_ml_function_average_pooling_node_saveState(const mlfunction_node *fnode, FILE *file) {
    auto f = (FnAveragePooling *) fnode->function;
    f->saveState(file);
}
...
void math21_ml_function_average_pooling_node_resize(
        mlfunction_node *fnode, const mlfunction_net *fnet, int nr_X, int nc_X) {
    auto f = (FnAveragePooling *) fnode->function;
    f->resize(nr_X, nc_X);
    math21_ml_function_average_pooling_node_reset(fnode);
}

void math21_ml_function_average_pooling_node_forward(
        mlfunction_node *fnode, mlfunction_net *net, mlfunction_node *finput) {
    auto f = (FnAveragePooling *) fnode->function;
    f->forward(net, finput);
}

void math21_ml_function_average_pooling_node_backward(
        mlfunction_node *fnode, mlfunction_net *net, mlfunction_node *finput) {
    auto f = (FnAveragePooling *) fnode->function;
    f->backward(finput);
}
...
void math21_ml_function_average_pooling_node_log(const mlfunction_node *fnode, const char *varName) {
    auto f = (const FnAveragePooling *) fnode->function;
    f->log(varName);
}
...
void math21_ml_function_average_pooling_node_reset(mlfunction_node *fnode) {
    FnAveragePooling *f = (FnAveragePooling *) fnode->function;
...
FnAveragePooling *
math21_ml_function_average_pooling_create(mlfunction_node *fnode, int batch, int c, int h, int w) {
    auto f = new FnAveragePooling();
    f->create(batch, c, h, w);
    if (fnode) {
        ...
        fnode->resize = math21_ml_function_average_pooling_node_resize;
