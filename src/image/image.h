...
namespace math21 {
    ...
    void math21_img_resize(const MatR &src, MatR &dst, NumN img_resize_method = m21_flag_resize_method_default);

    void math21_img_resize(const Seqce <TenR> &srcs, Seqce <TenR> &dsts,
                           NumN img_resize_method = m21_flag_resize_method_default);

    void math21_img_resize(Seqce <TenR> &images, const VecN &d, NumN img_resize_method = m21_flag_resize_method_default);

    void math21_img_resize(TenR &image, const VecN &d, NumN img_resize_method = m21_flag_resize_method_default);
    ...
    void math21_img_planar_to_interleaved(const Tensor <T> &A, Tensor <S> &B) {
        ...
        math21_op_matrix_like_transpose(A, B, nch, nr * nc);
    }
    ...
    void math21_img_interleaved_to_planar(const Tensor <T> &A, Tensor <S> &B) {
        ...
        math21_op_matrix_like_transpose(A, B, nr * nc, nch);
    }
    ...
    namespace detail {
        ...
        void math21_image_argb8888_to_tensor_cuda(const NumN32 *data, TenR &m, NumN nr, NumN nc, NumN nch);

#endif
    }

    void math21_image_argb8888_to_tensor(const NumN32 *data, TenR &m, NumN nr, NumN nc, NumN nch = 4);
    ...
    template<typename T>
    void math21_device_image_pixel_value_set(T *data, NumN ir, NumN ic, NumN ich,
                                           NumN nr, NumN nc, NumN nch,
                                           T val) {
        ich-=1;
        ir-=1;
        math21_tool_assert(ir < nr && (ic-1) < nc && ich < nch);
        data[ich * nr * nc + ir * nc + ic] = val;
    }
    ...
    void math21_image_set_pixel(Tensor<T> &m, const VecN &pixel) {
        ...
        Seqce<Tensor<T>> ms;
        math21_operator_share_remove_dim_1(m, ms);
        for (NumN i = 1; i <= ms.size(); ++i) {
            math21_op_vector_set_value(ms(i), pixel(i));
        }
    }
    ...
    NumR math21_image_measure_similarity(const TenR &x0, const TenR &y0, NumN scale = 0, NumR pixelEpsilon = 0);
}