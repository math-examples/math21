...
namespace math21 {
    ...
    // copy from gpu
    void AutoBuffer::dataCopyFromGpu(PtrN8InWrapper data) {
    ...
    NumN *math21_memory_create_refcount() {
        NumN *refcount = 0;
    ...
    void math21_memory_getSpace(const SpaceParas &src, SpaceParas &dst, NumN offset, NumN size,
                                NumN unit) {
    ...
    void math21_memory_setSpace_cpu(SpaceParas &paras, void *byte,
                                    NumN nBytes, NumB hasRef, NumN type) {
        if (!hasRef)paras.ref_count = 0;
        else {
            paras.ref_count = math21_memory_create_refcount();
            MATH21_ASSERT(paras.ref_count);
        }
        paras.size = nBytes;
        paras.unit = 1;
        paras.type = type;
        paras._deviceType = m21_device_type_default;
        paras.address = (char *) byte;
        paras.start = (char *) byte;
    }

    void math21_memory_setSpace_wrapper(SpaceParas &paras, PtrVoidWrapper byte,
                                        NumN nBytes, NumB hasRef, NumN type) {
        if (!hasRef)paras.ref_count = 0;
        else {
            paras.ref_count = math21_memory_create_refcount();
        ...
        paras.start = (char *) byte;
#else
        paras._deviceType = m21_device_type_gpu;
        paras.address_wrapper = (PtrN8Wrapper) byte;
        paras.start_wrapper = (PtrN8Wrapper) byte;
#endif
    }
...
//            xjmemset(space_address, 0, sizeof(char) * nn);
                ...
                space_size = nn;
                if (space_address) {
                    refcount = math21_memory_create_refcount();
                    MATH21_ASSERT(refcount);
                } else {
                    refcount = 0;
                }
            }
            ...
            if (math21_vector_isEmpty_wrapper(space_address_wrapper)) {
                space_address_wrapper = (PtrN8Wrapper) math21_vector_create_buffer_wrapper(sizeof(char) * nn,
                                                                                               1);
...
#if !defined(MATH21_FLAG_USE_CPU)
                        space_address_wrapper = (PtrN8Wrapper) math21_vector_resize_buffer_wrapper(
                                (PtrVoidWrapper) space_address_wrapper, n, sizeof(char));
                        space_start_wrapper = space_address_wrapper;
#else
                        math21_tool_assert(0);
#endif
...
//        m21logDebug(*refcount);
    ...
    PtrVoidWrapper AutoBuffer::getObjGpu() const {
...
#if !defined(MATH21_FLAG_USE_CPU)
            PtrN8Wrapper data;
            data = (PtrN8Wrapper) autoBuffer.getObjGpu();
        ...
        if (isAddressEmpty()) {
            ...
            MATH21_ASSERT((refcount == 0 || (refcount != 0 && *refcount > 0)),
                          "data is supposed to be not null, but"
                                  ...
                                  << "\n\t*refcount is " << *refcount
                                  << "\n\tsize is " << size()
                                  << " (size is allowed not to be zero when is empty.)"
    ...
    void AutoBuffer::swap(AutoBuffer &B) {
        xjswap(space_address, B.space_address);
        xjswap(space_start, B.space_start);
        xjswap(space_size, B.space_size);
        xjswap(nn, B.nn);
        xjswap(refcount, B.refcount);
        xjswap(type, B.type);
        xjswap(_deviceType, B._deviceType);
#if !defined(MATH21_FLAG_USE_CPU)
        xjswap(space_address_wrapper, B.space_address_wrapper);
        xjswap(space_start_wrapper, B.space_start_wrapper);
#endif
