...
//            MATH21_ASSERT(0, "can't call this in current version!");
        ...
        // a is a kind of map but sorted. todo: make a sorted.
        ...
        TensorView(const Tensor<T> &m, const Seqce<VecN> &index) : Tensor<T>(), m(m) {
            ...
            for (NumN i = 1; i <= a.size(); ++i) {
                //0 means use all. todo: can use empty to denote all.
                if (index(i).size() == 1 && index(i)(1) == 0) {
                    a.at(i).setSize(m.dim(i));
                    a.at(i).letters();
                } else {
                    // todo: sort a(i), see error in test_tensor_submatrix
                    // If not sort a, a will be a map. But a must be sorted because this is tensor view.
        ...
        // b is a kind of shape mask, but may not be binary, and non-zero element denotes removing.
        ...
        TensorView(const Tensor<T> &_m, const VecN &index) : Tensor<T>(), m(_m) {
            ...
            b.assign(index);
            VecN d;
            math21_operator_tensor_shrink_shape_using_shape(m.shape(), b, d);
            this->setSizeNoSpace(d);
        }
