...
namespace math21 {
    ...
    // d_dst.size() >= d_ori.size()
    // d_dst is destination shape.
    template<typename VecType1, typename VecType2>
    NumB math21_operator_tensor_can_broadcast_to(const VecType1 &d_ori, const VecType2 &d_dst, NumB &noNeedTo) {
        if (d_ori.size() > d_dst.size())return 0;
        if (d_ori.size() == 0)return 0;
        MATH21_ASSERT(math21_operator_container_multiply_all(d_dst) != 0, "0 shape not supported in this version!");
        ...
        for (NumN i = 1; i <= d_ori.size(); ++i) {
            if (d_dst(i) != d_ori(i)) {
                noNeedTo = 0;
                if (d_ori(i) != 1)return 0;
            }
        }
        if (noNeedTo) {
            NumN n_1 = math21_operator_container_number_of_equal(1, d_dst, d_ori.size());
            if (n_1 != d_dst.size() - d_ori.size())noNeedTo = 0;
        }
        return 1;
    }

    // 1*2     compatible to 2*2*3, but not vice versa.
    // 1*2*3*1 compatible to 2*2*3, but not vice versa.
    // d is destination shape.
    template<typename VecType1, typename VecType2>
    NumB math21_broadcast_is_compatible_to(const VecType1 &d_ori, const VecType2 &d_dst) {
        NumB noNeedTo;
        if (d_dst.size() >= d_ori.size()) {
            return math21_operator_tensor_can_broadcast_to(d_ori, d_dst, noNeedTo);
        } else {
            VecN d;
            d.setSize(d_ori.size());
            d = 1;
            math21_operator_container_set_partially(d_dst, d, 0, 0, d_dst.size());
            return math21_operator_tensor_can_broadcast_to(d_ori, d, noNeedTo);
        }
    }

    void math21_operator_tensor_make_shape_standard(VecN &d);

    // standard(d1) = standard(d2), see math21_operator_tensor_make_shape_standard
    template<typename VecType1, typename VecType2>
