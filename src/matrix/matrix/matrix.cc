...
namespace math21 {
    ...
    void math21_tensor_log_wrapper(const char *name, PtrR32InWrapper data, const VecN &d) {
    ...
    NumB math21_rawtensor_convert_to_TenR_with_data_shared(const m21rawtensor &rawtensor, TenR &A) {
        ...
        math21_operator_tensor_set_size_cpu(A, d, rawtensor.data, 0);
        return 1;
    }
...
void math21_tensor_1d_float_log_wrapper(const char *name, PtrR32InWrapper data, NumN d1) {
    VecN d(1);
    d = d1;
    math21_tensor_log_wrapper(name, data, d);
}

void math21_tensor_2d_float_log_wrapper(const char *name, PtrR32InWrapper data, NumN d1, NumN d2) {
    VecN d(2);
    d = d1, d2;
    math21_tensor_log_wrapper(name, data, d);
}

void math21_tensor_3d_float_log_wrapper(const char *name, PtrR32InWrapper data, NumN d1, NumN d2, NumN d3) {
    VecN d(3);
    d = d1, d2, d3;
    math21_tensor_log_wrapper(name, data, d);
}

void math21_tensor_4d_float_log_wrapper(const char *name, PtrR32InWrapper data, NumN d1, NumN d2, NumN d3,
                                        NumN d4) {
    VecN d(4);
    d = d1, d2, d3, d4;
    math21_tensor_log_wrapper(name, data, d);
...
void
math21_tensor_5d_float_log_wrapper(const char *name, PtrR32InWrapper data, NumN d1, NumN d2, NumN d3, NumN d4,
                                   NumN d5) {
    VecN d(5);
    d = d1, d2, d3, d4, d5;
    math21_tensor_log_wrapper(name, data, d);
