...
//            MATH21_ASSERT(N > 0, "x-D tensor, and x must be positive integer");
        ...
        void _setSizeDataFixed(const ArrayN &_d) {
            MATH21_ASSERT(volume() == math21_operator_container_multiply_all(_d),
                          "data size check failed!"
                                  << "\nvolume() = " << volume()
                                  << "\nmultiply_all(_d) = "
                                  << math21_operator_container_multiply_all(_d)
                                  << "\n_d = " << _d
            );
            _setSize_shape(_d, 0);
            if (isEmpty()) {
        ...
        NumB isLogAutoBuffer() const {
            return math21_global_tensor_is_log_auto_buffer();
        }
        ...
        NumN index_tensor_to_array(NumN j1, NumN j2, NumN j3) const {
            ...
            MATH21_ASSERT(
                    j1 >= 1 && j1 <= dim(1) && j2 >= 1 && j2 <= dim(2) && j3 >= 1 && j3 <= dim(3),
                    "\tYou must give a valid index");
...
//            MATH21_ASSERT(0 && "test")
        ...
        // reshape not sharing. See math21_operator_share_reshape
        ...
        NumB isVectorInMath() const { // row vector or column vector
            if (0 < dims() && dims() <= 2) {
                if (nrows() == 1 || ncols() == 1) {
                    return 1;
                }
        ...
        NumN nrows() const {
            if (isEmpty()) { // shape is 2*0 => nr = 0, dim(1) = 2
                return 0;
            }
        ...
        NumB log(const char *name = 0, NumB isUsingCommas = 0, NumB isLogDetail = 1,
                 NumN precision = 3) const;

        NumB
        log(std::ostream &io, const char *name = 0, NumB isUsingCommas = 0, NumB isLogDetail = 1,
            NumN precision = 3) const;
        ...
        Tensor<T> &operator=(Tensor<T> &&B) {
            m21warndebug("In operator=(Tensor&&).");
            if (this != &B) {
                swap(B);
                B.clear();
            }
        ...
        PtrN8Wrapper getDataAddressWrapper() {
            MATH21_ASSERT(isContinuous())
            SpaceParas paras = getSpace();
#if defined(MATH21_FLAG_USE_CPU)
            MATH21_ASSERT(is_cpu())
            return (PtrN8Wrapper) paras.start;
#else
            MATH21_ASSERT(!is_cpu())
            return (PtrN8Wrapper) paras.start_wrapper;
#endif
        }

        PtrN8InWrapper getDataAddressWrapper() const {
            MATH21_ASSERT(isContinuous())
            SpaceParas paras = getSpace();
#if defined(MATH21_FLAG_USE_CPU)
            MATH21_ASSERT(is_cpu())
            return (PtrN8InWrapper) paras.start;
#else
            MATH21_ASSERT(!is_cpu())
            return (PtrN8InWrapper) paras.start_wrapper;
#endif
        }
...
//            assign((const T&) val);
        ...
        void swap(Tensor &B) {// not recommended when using with shared memory
            MATH21_ASSERT(getClassName() == Tensor::getClassName(), "You must overwrite to use");
            data.swap(B.data);
            a.swap(B.a);
            d.swap(B.d);
            xjswap(N, B.N);
            xjswap(is_column_major, B.is_column_major);
        }
...
//        shape().log("shape", 0);
    ...
    template<typename T>
    NumB
    Tensor<T>::log(const char *name, NumB isUsingCommas, NumB isLogDetail, NumN precision) const {
        NumB flag;
#ifdef MATH21_ANDROID
        SimpleStreamBuf ssb;
        std::streambuf *backup;
        ...
        std::cout.rdbuf(&ssb);
#endif
        flag = log(std::cout, name, isUsingCommas, isLogDetail, precision);
#ifdef MATH21_ANDROID
        std::cout.rdbuf(backup);
#endif
        return flag;
    ...
    template<typename T>
    NumB Tensor<T>::log(std::ostream &io, const char *name,
                        NumB isUsingCommas, NumB isLogDetail, NumN precision) const {
        if (isLogDetail) {
            logInfo(io, name);
        } else {
            if (name) {
