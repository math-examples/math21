...
namespace math21 {
    ...
    void math21_operator_container_create_with_increment(VecR &v, NumR from, NumR inc, NumR to, NumR epsilon, NumN n) {
        MATH21_ASSERT(inc != 0);
        if (n == 0) {
            NumZ n0 = (NumZ)std::floor((to - from) / inc + epsilon) + 1;
            MATH21_ASSERT(n0 > 0);
            n = n0;
        }
        v.setSize(n);
    ...
    void math21_matlab_diag(const MatR &A, MatR &B, NumZ n) {
        math21_operator_matrix_diag_do(A, B, n);
    }

    /*
     * see math21_geometry_rodrigues
# References
 ...
 * */
    void math21_la_convert_RodriguesForm_to_rotation(const VecR &w, MatR &R) {
        ...
        wy = w(2);
        wz = w(3);
        MatR omega(3, 3); // see math21_geometry_cross_product
        omega =
                0, -wz, wy,
                wz, 0, -wx,
                -wy, wx, 0;
