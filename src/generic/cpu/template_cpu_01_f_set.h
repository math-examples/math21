...
namespace math21 {
    ...
    template<typename T>
    void math21_template_tensor_subregion_f_set_or_get_cpu(NumN fname, NumN n, T *x, T *y, NumN dims,
...
#pragma omp parallel for
        for (id = 1; id <= n; ++id) {
            math21_template_tensor_subregion_f_set_or_get_cpu_kernel(
                    f, n, x, y, dims, dx, dy, offset, isGet, id);
        }
    }


    template<typename T>
    void math21_template_matrix_f_set_by_matrix_cpu(NumN fname, NumN d1, NumN d2,
...
#pragma omp parallel for
        for (id = 1; id <= n; ++id) {
            math21_template_matrix_f_set_by_matrix_cpu_kernel(
                    f,
...
#pragma omp parallel for
    ...
    template<typename T>
    void math21_template_vector_f_set_by_value_cpu(NumN fname, NumN n, T value, T *x, NumN stride_x) {
        x -= 1;
        NumN id;
        auto f = math21_generic_fname_to_f_addto_like_name_cpu(fname);
#pragma omp parallel for
        for (id = 1; id <= n; ++id) {
            math21_template_vector_f_set_by_value_cpu_kernel(
                    f, n, value, x, stride_x, id);
        }
    }
}