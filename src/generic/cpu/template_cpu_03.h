...
#pragma omp parallel for
    ...
    template<typename T1, typename T2>
    void math21_template_matrix_trans_reverse_axis_cpu(NumN n, const T1 *x, T2 *y, NumN d1, NumN d2, NumB isXAxis) {
        x -= 1;
        y -= 1;
        NumN id;
        if (isXAxis) {
#pragma omp parallel for
            for (id = 1; id <= n; ++id) {
                math21_template_matrix_trans_reverse_axis_cpu_kernel(n, x, y, d1, d2, id);
            }
        } else {
#pragma omp parallel for
            for (id = 1; id <= n; ++id) {
                math21_template_matrix_trans_reverse_y_axis_cpu_kernel(n, x, y, d1, d2, id);
            }
        }
    }
    ...
    template<typename T1, typename T2>
    void
    math21_template_tensor_swap_axes_24_in_d5_cpu(const T1 *x, T2 *y, NumN d1, NumN d2, NumN d3, NumN d4, NumN d5) {
        x -= 1;
        y -= 1;
