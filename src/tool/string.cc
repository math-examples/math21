...
namespace math21 {
    void math21_string_argv_2_string(int &argc, const char **&argv, SeqStr &strs, NumN offset) {
        NumZ n = argc - offset;
        if (n <= 0) {
            strs.clear();
            return;
    ...
    void math21_string_2_argv(const SeqStr &strs, int &argc, const char **&argv, NumN offset) {
    ...
    void math21_string_2_argv(const SeqStr &strs, int &argc, char **&argv, NumN offset) {
    ...
    void math21_operator_tensor_set_string(TenN8 &A, NumN n, const NumN8 *s, NumB shallow) {
        if (shallow) {
            math21_operator_tensor_set_data_cpu(A, n, s);
        } else {
            math21_operator_tensor_copy_data_cpu(A, n, s);
        }
    }

    void math21_operator_tensor_set_string(TenN8 &A, const std::string &s, NumB shallow) {
        if (shallow) {
            math21_operator_tensor_set_data_cpu(A, s.size(), (const NumN8 *) s.c_str());
        } else {
            math21_operator_tensor_copy_data_cpu(A, s.size(), (const NumN8 *) s.c_str());
        }
    }

    void math21_operator_tensor_get_string(const TenN8 &A, std::string &s) {
        math21_tool_std_string_resize(s, A.size());
        math21_memory_tensor_data_copy_to_buffer_cpu(A, reinterpret_cast<NumN8 *>(&s[0]));
    }
