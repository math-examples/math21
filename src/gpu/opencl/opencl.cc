...
cl_device_type math21_opencl_get_device_gpu_type() {
//    cl_device_type device_type =
//            CL_DEVICE_TYPE_GPU | CL_DEVICE_TYPE_ACCELERATOR; // some leads to error CL_INVALID_DEVICE_TYPE
    cl_device_type device_type = CL_DEVICE_TYPE_GPU;
    return device_type;
}

std::string math21_opencl_error2string(cl_int error) {
#define MATH21_LOCAL_F(a) case a: return #a;
    switch (error) {
        MATH21_LOCAL_F(CL_MEM_OBJECT_ALLOCATION_FAILURE)
        MATH21_LOCAL_F(CL_BUILD_PROGRAM_FAILURE)
        MATH21_LOCAL_F(CL_MISALIGNED_SUB_BUFFER_OFFSET) // OpenCL driver may or may not enforce the alignment rule.
        MATH21_LOCAL_F(CL_INVALID_DEVICE_TYPE)
        MATH21_LOCAL_F(CL_INVALID_MEM_OBJECT)
        MATH21_LOCAL_F(CL_INVALID_BUILD_OPTIONS)
        MATH21_LOCAL_F(CL_INVALID_ARG_INDEX)
        MATH21_LOCAL_F(CL_INVALID_ARG_SIZE)
        MATH21_LOCAL_F(CL_INVALID_BUFFER_SIZE)
        default:
            return math21_string_to_string(error);
    }
...
void math21_opencl_checkError(cl_int error) {
    if (error != CL_SUCCESS) {
        std::string message = math21_opencl_error2string(error);
