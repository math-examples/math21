...
void math21_opencl_test3() {
    ...
    for (size_t p = 0; p < num_platforms; ++p) {
        ...
        for (size_t d = 0; d < num_devices; ++d) {
            ...
            cl_uint n_bits;
            clGetDeviceInfo(device, CL_DEVICE_MEM_BASE_ADDR_ALIGN, sizeof(n_bits), &n_bits, NULL);
            printf("  CL_DEVICE_MEM_BASE_ADDR_ALIGN:\t%u bits\n", n_bits);
            clGetDeviceInfo(device, CL_DEVICE_MIN_DATA_TYPE_ALIGN_SIZE, sizeof(n_bits), &n_bits, NULL);
            printf("  CL_DEVICE_MIN_DATA_TYPE_ALIGN_SIZE:\t%u bytes\n", n_bits);
...
int math21_opencl_test_NumR64()
{
    ...
    const char* src_not_variadic =
            ...
            "kernel void test(global double* input, global double* output) \n"
            "{                                                       \n"
            "    int i = get_global_id(0);                           \n"
    ...
    input_buf = clCreateBuffer(context, CL_MEM_READ_ONLY, 16*sizeof(cl_double), NULL, NULL);
    output_buf = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 16*sizeof(cl_double), NULL, NULL);
    if(!input_buf || !output_buf)
    {
        printf("error: clCreateBuffer\n");
        return EXIT_FAILURE;
    ...
    err = clEnqueueWriteBuffer(command_queue, input_buf, CL_TRUE, 0, 16*sizeof(cl_double), input, 0, NULL, NULL);
    if(err != CL_SUCCESS)
    {
        printf("error: clEnqueueWriteBuffer\n");
        return EXIT_FAILURE;
    ...
    err = clEnqueueReadBuffer(command_queue, output_buf, CL_TRUE, 0, 16*sizeof(cl_double), output, 0, NULL, NULL );
    if(err != CL_SUCCESS)
    {
        printf("error: clEnqueueReadBuffer\n");
        return EXIT_FAILURE;
