...
//            clFinish(*queue);// added by ye
            error = clReleaseCommandQueue(
                    *queue); // seems that it can't be called after main exits. so call math21_opencl_destroy() before main exits.
            if (error != CL_SUCCESS) {
    ...
    void m21cltool::init_device(int gpuIndex, cl_platform_id &platform_id, cl_device_id &device) {
        ...
        cl_uint num_devices;
        cl_device_type deviceType = math21_opencl_get_device_gpu_type();
        error = clGetDeviceIDs(platform_id, deviceType, 0, 0, &num_devices);
        if (error != CL_SUCCESS) {
            MATH21_ASSERT(0, "Error getting OpenCL device ids: " + errorMessage(error));
        }

        cl_device_id *device_ids = new cl_device_id[num_devices];
        error = clGetDeviceIDs(platform_id, deviceType, num_devices, device_ids,
                               &num_devices);
        if (error != CL_SUCCESS) {
            MATH21_ASSERT(0, "Error getting OpenCL device ids: " + errorMessage(error));
        }
...
//        *queue = clCreateCommandQueueWithProperties(*context, device, 0, &error);
    ...
    m21cltool *m21cltool::createForIndexedGpu_raw(int gpu) {
        ...
        if (math21_global_is_debug()) {
            m21log("num_platforms", num_platforms);
        }
        for (int platform = 0; platform < (int) num_platforms; platform++) {
            ...
            cl_uint num_devices;
            cl_device_type deviceType = math21_opencl_get_device_gpu_type();
            error = clGetDeviceIDs(platform_id, deviceType, 100, device_ids,
                                   &num_devices);
            if (error != CL_SUCCESS) {
                if (math21_global_is_debug()) {
                    m21log("error", errorMessage(error));
                }
                continue;
            }
            if (math21_global_is_debug()) {
                m21log("num_devices", num_devices);
            }
    ...
    std::string m21cltool::errorMessage(cl_int error) {
        return math21_opencl_error2string(error);
    }
