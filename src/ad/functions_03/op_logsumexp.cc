...
namespace math21 {
    namespace ad {
        ...
        // y = f(x) => dy/dx = exp(x-y)
        VarAd op_logsumexp::df_vjp(const SetVar &X, VarAd x, VarAd y, VariableMap &data) const {
            ...
            op_multiply mul0;
            Function &mul = mul0;
            VarAd my = mul.evaluate(k, y, data);
            ...
            op_add add0;
            Function &add = add0;
            VarAd xmy = add.evaluate(x, my, data);
            op_exp exp0;
            Function &exp = exp0;
            VarAd df = exp.evaluate(xmy, data);
            auto &value_df = data.at(df).getValue();
            MATH21_ASSERT_CODE(value_df.isSameSize(data(x).getValue().shape()))
            return df;
        }

        VarAd op_logsumexp::cr_vjp_inner(const SetVar &X, VarAd x, VarAd y, VarAd dy, VariableMap &data) const {
