...
namespace math21 {
    namespace ad {
        VarAd op_repeat::cr_vjp_inner(const SetVar &X, VarAd x, VarAd y, VarAd dy, VariableMap &data) const {
        ...
        VarAd op_undo_repeat_sum::cr_vjp_inner(const SetVar &X, VarAd x, VarAd y, VarAd dy, VariableMap &data) const {
            NumN pos = math21_operator_container_arg(X, x);
            MATH21_ASSERT(pos == 1)
            op_repeat _;
            Function &f = _;
