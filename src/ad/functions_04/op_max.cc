...
namespace math21 {
    namespace ad {
        VarAd op_max::cr_vjp_inner(const SetVar &X, VarAd x, VarAd y, VarAd dy, VariableMap &data) const {
            MATH21_ASSERT(0, "todo")
            return VarAd(0);
        }

        // X = {x, axes, isKeepingDims}
        void op_max::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
