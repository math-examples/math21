    ...
    namespace ad {
        namespace rnn_detail {
            void split_rnn_params(const PointAd &params_point, ad_rnn_params &params,
                ...
                PointAd &params_1d = params.params_1d;
                PointAd &init_hiddens = params.init_hiddens;
                PointAd &change_para = params.change_para;
                PointAd &predict_para = params.predict_para;
            ...
            void split_lstm_params(const PointAd &params_point, ad_lstm_params &params,
                ...
                PointAd &change_para = params.change_para;
                PointAd &predict_para = params.predict_para;
                PointAd &init_cells = params.init_cells;
                PointAd &forget_para = params.forget_para;
                PointAd &ingate_para = params.ingate_para;
                PointAd &outgate_para = params.outgate_para;
...
// output: num_sequences * output_size
        ...
        PointAd ad_rnn_part_log_likelihood(const PointAd &logprobs0, const PointAd &data_targets) {
    ...
    void math21_data_text_lstm_generate_text(const TenR &x_value_final,
        ...
        for (NumN i = 1; i <= n_lines; ++i) {
            ...
            for (NumN j = 1; j <= sequence_length; ++j) {
                TenN8 line_c;
                math21_operator_tensor_set_string(line_c, (NumN) text.size(), (const NumN8 *) text.c_str());
                VecR line;
                ...
                ad_clear_graph();
                auto data_inputs = PointAd (data, 0, deviceType); // data may have shape (0, 1, 128)
                auto params = PointAd (x_value_final, 0, deviceType);
...
//        m21log(text);
    ...
    void math21_data_text_lstm_print_training_prediction(const TenR &x_cur, void *data) {
        ...
        PointAd px = args->x;
        PointAd plogprobs = args->logprobs;
        PointAd pdata_targets = args->data_targets;
        ...
        for (NumN i = 1; i <= logprobs.dim(2); ++i) {
            ...
            std::string s1, s2;
            math21_operator_tensor_get_string(training_text, s1);
            math21_operator_tensor_get_string(predicted_text, s2);
...
//        printf("Iteration %d Train loss: %lf\n", iter, y(1));
    ...
    void math21_ml_lstm_text(const m21lstm_text_type_config &config) {
        ...
        // prepare data
        TenR data;
        flag = math21_data_text_build_dataset(textPath.c_str(), data, config.time_steps, config.max_batches,
                                              config.alphabet_size);
        if (!flag)return;

        PointAd data_inputs(data, 0, config.deviceType);
        PointAd data_targets(data, 0, config.deviceType);
...
//        NumN n_paras = ad_rnn_calculate_params_size(config.input_size, config.state_size, config.output_size);
        ...
        VecR params_value(n_paras);
        PointAd params(params_value, 1, ad_get_device_type(data_inputs));
...
//                                   input_size, state_size, output_size);
        ...
        if (config.debug && config.functionParasPath_init.empty() && !math21_global_is_data_aligned()) {
            y.log("y", 15);
        ...
        OptParasAdam parasAdam;
        parasAdam.num_iters = config.num_iters;
        parasAdam.step_size = config.step_size;
        math21_opt_adam(x_value_cur, df_1d, &grad_args, callback, &callback_args,
                        parasAdam);

//        SinDf1dCallback cbCompute(&grad_args);
//        SinPrintCallback cbPrint(&callback_args);
//        OptAlg optAlg;
//        optAlg.set(OptUpdateType_Adam, &parasAdam, &cbCompute, &cbPrint);
//        optAlg.run(x_value_cur);
