...
namespace math21 {
    ...
    namespace ad {
        ...
        PointAd ad_rnn_sin01_predict(PointAd x, const PointAd &theta_rnn, const PointAd &w, PointAd b,
...
//        time_steps = 100;
        ...
        if (math21_gpu_is_available()) {
            deviceType = m21_device_type_gpu;
            m21log("device: gpu");
        } else {
...
#undef MATH21_LOCAL_F
    ...
    static void f_1d(const TenR &x, TenR &y, NumN iter, void *data) {
        ...
        PointAd px = args->x;
        PointAd py = args->y;
        PointAd data_inputs = args->data_inputs;
        PointAd data_targets = args->data_targets;
    ...
    static void df_1d(const TenR &x, TenR &dy, NumN iter, void *data) {
        auto args = (const lstm_f_pair_args *) data;
        PointAd px = args->x;
        PointAd pdy = args->y;
        ad_get_value(px) = x;
        ad_fv(pdy);
    ...
    class SinDf1dCallback : public OptCallbackAdam {
    private:
        void *data;
    public:
        SinDf1dCallback(void *data) : data(data) {}

        NumB compute(const TenR &x, TenR &dy, NumN iter) const override {
            df_1d(x, dy, iter, data);
            return 1;
        }
    };
...
//        math21_data_text_lstm_print_training_prediction(x_cur, data);
    ...
    class SinPrintCallback : public OptPrintCallbackAdam {
    private:
        void *data;
    public:
        SinPrintCallback(void *data) : data(data) {}

        void log(const TenR &x_cur, const TenR &gradient, NumN iter) const override {
            callback(x_cur, gradient, iter, data);
        }
    };
    ...
    void math21_ml_lstm_sin01(const m21lstm_sin01_type_config &config) {
        ...
        PointAd data_inputs(Y2, 0, config.deviceType);
        PointAd data_targets(FY, 0, config.deviceType);
