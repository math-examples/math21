...
m21point math21_test_ad_get_f_rnn_predict(
    ...
    m21point y_empty = {0};
    if (math21_type_get<PointAd >() != params.type
        || math21_type_get<PointAd >() != data_inputs.type) {
        m21warn("cast error");
        return y_empty;
    }
    auto py = ad_rnn_predict(
            math21_cast_to_T<PointAd >(params),
            math21_cast_to_T<PointAd >(data_inputs),
            input_size,
            state_size,
            output_size);
    auto y = new PointAd ();
...
m21point math21_test_ad_get_f_rnn_part_log_likelihood(
    ...
    m21point y_empty = {0};
    if (math21_type_get<PointAd >() != logprobs.type
        || math21_type_get<TenR>() != data_targets.type) {
        m21warn("cast error");
        return y_empty;
    }
    auto py = ad_rnn_part_log_likelihood(
            math21_cast_to_T<PointAd >(logprobs),
