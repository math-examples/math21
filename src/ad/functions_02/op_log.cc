...
namespace math21 {
    namespace ad {
        ...
        VarAd op_log::df_vjp(const SetVar &X, VarAd x, VarAd y, VariableMap &data) const {
        ...
        // (a^x)' = ln(a) * a^x
        // log(a, x)' = (1/ln(a)) * (1/x)
        void op_log::df(const SetVar &X, VarAd x, VarAd y, VarAd &dydx, VariableMap &data) const {
            if (X(1) != x) {
        ...
        void op_log::df_dbr(const SetVar &X, VarAd x, VarAd y, VarAd &dydx, VariableMap &data) const {
            if (X(1) != x) {
                dydx = ad_global_get_constant_0();
                return;
            }
