...
namespace math21 {
    namespace ad {
        ...
        VarAd op_broadcast_tensor::cr_vjp_inner(const SetVar &X, VarAd x, VarAd y, VarAd dy, VariableMap &data) const {
        ...
        void op_broadcast_tensor::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
            math21_tool_assert(X.size() == 2);
            VarAd y = Y(1);
            const auto &x_value = data.at(X(1)).getValue();
            const auto &d_value = data.at(X(2)).getValue();
        ...
        VarAd op_create::cr_vjp_inner(const SetVar &X, VarAd x, VarAd y, VarAd dy, VariableMap &data) const {
            math21_tool_assert(X.size() == 2);
            math21_tool_assert(X(1) == x);
            op_sum _;
            Function &f = _;
        ...
        void op_create::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
