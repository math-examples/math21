...
namespace math21 {
    namespace ad {
        ...
        VarAd op_power::cr_vjp_inner(const SetVar &X, VarAd x, VarAd y, VarAd dy, VariableMap &data) const {
            math21_tool_assert(X.size() == 1);
            math21_tool_assert(X(1) == x);
            if (k == 0 || p == 0) {
                auto &dy_vec = data.at(dy).getValue();
                VarAd dx = data.createC("0");
            ...
            VarAd y_power = power.evaluate(x, data);
            op_multiply multiply0;
            Function &multiply = multiply0;
            return multiply.evaluate(dy, y_power, data);
        }

        void op_power::df(const SetVar &X, VarAd x, VarAd y, VarAd &dydx, VariableMap &data) const {
            if (X(1) != x) {
        ...
        void op_power::df_dbr(const SetVar &X, VarAd x, VarAd y, VarAd &dydx, VariableMap &data) const {
            if (X(1) != x) {
                dydx = ad_global_get_constant_0();
                return;
            }
