...
PointAd ad_sin_taylor_appr(const PointAd &x) {
    auto currterm = x;
    auto ans = currterm;
...
m21point math21_test_ad_get_f_gmm_log_likelihood_with_order(m21point params0, m21point data0,
    ...
    m21point y_empty = {0};
    if (math21_type_get<PointAd>() != params0.type || math21_type_get<TenR>() != data0.type) {
        m21warn("cast error");
        return y_empty;
    }
    auto &params = math21_cast_to_T<PointAd>(params0);
    TenR &data = math21_cast_to_T<TenR>(data0);
...
m21point math21_point_ad_grad(m21point x, m21point y) {
    m21point y_empty = {0};
    if (math21_type_get<PointAd>() != x.type || math21_type_get<PointAd>() != y.type) {
        m21warn("cast error");
        return y_empty;
    }
    auto &px = math21_cast_to_T<PointAd>(x);
    auto &py = math21_cast_to_T<PointAd>(y);
    auto *dy = new PointAd();
    *dy = grad(px, py);
    if (dy->isEmpty()) {
        m21warn("id=0");
...
m21point math21_point_ad_hessian_vector_product(m21point x, m21point y, m21point v) {
    m21point y_empty = {0};
    if (math21_type_get<PointAd>() != x.type || math21_type_get<PointAd>() != y.type
        || math21_type_get<PointAd>() != v.type) {
        m21warn("cast error");
        return y_empty;
    }
    auto &px = math21_cast_to_T<PointAd>(x);
    auto &py = math21_cast_to_T<PointAd>(y);
    auto &pv = math21_cast_to_T<PointAd>(v);
    auto *dy = new PointAd();
...
void math21_point_ad_fv(m21point x) {
    ad_fv(math21_cast_to_T<PointAd>(x));
}
