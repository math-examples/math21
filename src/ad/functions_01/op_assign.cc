...
namespace math21 {
    namespace ad {
        ...
        VarAd op_assign::cr_vjp_inner(const SetVar &X, VarAd x, VarAd y, VarAd dy, VariableMap &data) const {
        ...
        VarAd op_assign::evaluate(const SetVar &X, VariableMap &data) {
            math21_tool_assert(X.size() == 1);
        ...
        void op_assign::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
            VarAd y = Y(1);
        ...
        void op_assign::df(const SetVar &X, VarAd x, VarAd y, SetVar &output, VariableMap &data) const {
            if (X(1) == x) {
                op_mat_eye matEye;
        ...
        void op_assign::cr(const SetVar &X, VarAd x, VarAd y, VarAd dy, SetVar &output, VariableMap &data) const {
            if (X(1) == x) {
                op_assign assign0;
                Function &assign = assign0;
                VarAd y_assign;
...
//                numConstant.f(X, output, data);
        ...
        void op_assign::backward(const SetVar &X, VarAd x, VarAd y, VarAd dy, SetVar &output, VariableMap &data) const {
            if (X(1) == x) {
                op_assign assign0;
                Function &assign = assign0;
                VarAd y_assign;
        ...
        void op_assign::f(const SetVar &X, SetVar &output, VariableMap &data) {
            MATH21_ASSERT(X.size() == 1)
            VarAd y = data.createV("op_assign(x)");
            data.at(y).setf(this);
            data.at(y).setX(X);
            for (NumN i = 1; i <= X.size(); ++i) {
                data.at(X(i)).addy(y);
        ...
        void op_assign::compute(const SetVar &X, const SetVar &Y, VariableMap &data, Derivative &derivative) {
        ...
        void op_assign::setSize(const SetVar &X, const SetVar &Y, VariableMap &data) const {
        }
