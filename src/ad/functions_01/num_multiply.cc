...
namespace math21 {
    namespace ad {
        ...
        void op_num_multiply::df(const SetVar &X, VarAd x, VarAd y, SetVar &output, VariableMap &data) const {
            MATH21_ASSERT(0)
        }

        void op_num_multiply::cr(const SetVar &X, VarAd x, VarAd y, VarAd dy, SetVar &output, VariableMap &data) const {
            if (!X.contains(x)) {
        ...
        void op_num_multiply::backward(const SetVar &X, VarAd x, VarAd y, VarAd dy, SetVar &output, VariableMap &data) const {
            if (!X.contains(x)) {
                output.set(ad_global_get_constant_0());
                return;
            }
        ...
        void op_num_multiply::f(const SetVar &X, SetVar &output, VariableMap &data) {
            if (ad_is_containing_constant_num_0(X, data)) {
                output.set(ad_global_get_constant_0());
                return;
            }
            VarAd y = data.createV("op_num_multiply(x)");
            data.at(y).setf(this);
            data.at(y).setX(X);
        ...
        void op_num_multiply::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
            if (X.isEmpty()) {
                return;
            }
            VarAd y = Y(1);
        ...
        void op_num_multiply::forward(const SetVar &X, SetVar &output, VariableMap &data) {
            if (X.isEmpty()) {
                return;
            }
            if (ad_is_containing_constant_num_0(X, data)) {
        ...
        void op_num_multiply::setSize(const SetVar &X, const SetVar &Y, VariableMap &data) const {
        }
