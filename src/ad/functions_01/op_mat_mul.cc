...
//                X.log("X");
//                dY.log("dY");
//                math21_operator_share_remove_dim_1(dY, dYs);
            math21_operator_share_mat_to_mats(dY, dYs, W.dim(1), X.dim(2));
//                dYs.log("dYs");
            dW.setSize(dY.dim(1), W.size());
            math21_operator_share_mat_to_mats(dW, dWs, W.dim(1), W.dim(2));
...
//                dYs.log("dYs");
            ...
            math21_operator_share_mat_to_mats(dX, dXs, X.dim(1), X.dim(2));
        ...
        VarAd op_mat_mul::cr_vjp_inner(const SetVar &X, VarAd x0, VarAd y, VarAd dy, VariableMap &data) const {
            ...
            VarAd x = X(2);
            if (x0 == w) {
                // todo: fusing the two transformations together if don't compute higher order derivatives.
                VarAd x_trans = mat_trans.evaluate(x, data);
                return mat_mul.evaluate(dy, x_trans, data);
            } else {
                VarAd w_trans = mat_trans.evaluate(w, data);
                return mat_mul.evaluate(w_trans, dy, data);
            }
        }

        VarAd op_mat_mul::evaluate(const SetVar &X, VariableMap &data) {
            math21_tool_assert(X.size() == 2);
        ...
        void op_mat_mul::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
            VarAd y = Y(1);
            VarAd w = X(1);
            VarAd x = X(2);
        ...
        void op_mat_mul::cr(const SetVar &X, VarAd x, VarAd y, VarAd dy, SetVar &output, VariableMap &data) const {
            if (X.size() == 0) {
                return;
            }
        ...
        void op_mat_mul::backward(const SetVar &X, VarAd x, VarAd y, VarAd dy, SetVar &output, VariableMap &data) const {
            if (X.size() == 0) {
                return;
            }
            MATH21_ASSERT(X.size() == 2, "not implemented")
        ...
        void op_mat_mul::f(const SetVar &X, SetVar &output, VariableMap &data) {
            if (X.size() == 0) {
                return;
            }
            if (X.size() == 1) {
        ...
        void op_mat_mul::compute(const SetVar &X, const SetVar &Y, VariableMap &data, Derivative &derivative) {
            if (Variable::isRequestingAbstractCompletely()) {
                fv(X, Y, data);
                return;
            }
            ...
            VarAd x = X(2);
            VarAd y = Y(1);
            const auto &w_mat = data(w).getMatVar();
        ...
        void op_mat_mul::setSize(const SetVar &X, const SetVar &Y, VariableMap &data) const {
    ...
    namespace ad {
        ...
        void op_mat_dmul::cr(const SetVar &X, VarAd x, VarAd y, VarAd dy, SetVar &output, VariableMap &data) const {
            MATH21_ASSERT(0)
        }

        void op_mat_dmul::f(const SetVar &X, SetVar &output, VariableMap &data) {
            ...
            VarAd y = data.createV(name.c_str());
            data.at(y).setf(this);
            data.at(y).setX(X);
            for (NumN i = 1; i <= X.size(); ++i) {
                data.at(X(i)).addy(y);
        ...
        // dot product
        void op_mat_dmul::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
...
//                                                  data.at(Y(1)).getValue(), _varId);
        ...
        void op_mat_dmul::setSize(const SetVar &X, const SetVar &Y, VariableMap &data) const {
            MATH21_ASSERT(0, "not implemented");
        }
