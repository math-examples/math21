...
namespace math21 {
    namespace ad {
        ...
        VarAd op_mat_trans::cr_vjp_inner(const SetVar &X, VarAd x, VarAd y, VarAd dy, VariableMap &data) const {
...
//            dy_mat.reshape(y_mat.shape());
        ...
        VarAd op_mat_trans::evaluate(const SetVar &X, VariableMap &data) {
            math21_tool_assert(X.size() == 1);
        ...
        void op_mat_trans::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
            VarAd y = Y(1);
            const auto &x_mat = data.at(X(1)).getValue();
            auto &y_mat = data.at(y).getValue();
//            math21_operator_matrix_trans(x_mat, y_mat);
            math21_op_matrix_transpose(x_mat, y_mat);
        }

        void op_mat_trans::cr(const SetVar &X, VarAd x, VarAd y, VarAd dy, SetVar &output, VariableMap &data) const {
            MATH21_ASSERT(0, "not implemented")
        }

        void op_mat_trans::f(const SetVar &X, SetVar &output, VariableMap &data) {
            if (X.size() == 0) {
                return;
            }
            MATH21_ASSERT(X.size() == 1)
            VarAd y = data.createV("mat_trans(x)");
            if (isSetSize()) {
        ...
        void op_mat_trans::setSize(const SetVar &X, const SetVar &Y, VariableMap &data) const {
            MATH21_ASSERT(0, "not implemented");
        }
