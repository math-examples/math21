...
namespace math21 {
    namespace ad {
        ...
        void op_vec_add::cr(const SetVar &X, VarAd x, VarAd y, VarAd dy, SetVar &output, VariableMap &data) const {
            VarAd dx = data.createC("1=d(op_vec_add(x))");
            data.setValue(dx, 1);
            if (isSetSize()) {
        ...
        void op_vec_add::f(const SetVar &X, SetVar &output, VariableMap &data) {
            if (X.size() == 0) {
                output.clear();
                return;
            }
            ...
            VarAd y = data.createV("op_vec_add(x)");
            if (isSetSize()) {
        ...
        void op_vec_add::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
            VarAd x1 = X(1);
            VarAd y = Y(1);
        ...
        void op_vec_add::setSize(const SetVar &X, const SetVar &Y, VariableMap &data) const {
