...
namespace math21 {
    namespace ad {
        ...
        void op_num_assign::df(const SetVar &X, VarAd x, VarAd y, SetVar &output, VariableMap &data) const {
            if (X(1) == x) {
        ...
        void op_num_assign::cr(const SetVar &X, VarAd x, VarAd y, VarAd dy, SetVar &output, VariableMap &data) const {
            if (X(1) == x) {
                SetVar input;
                input.add(dy);
        ...
        void op_num_assign::f(const SetVar &X, SetVar &output, VariableMap &data) {
            MATH21_ASSERT(X.size() == 1)
            VarAd y = data.createV("op_num_assign(x)");
            data.at(y).setf(this);
            data.at(y).setX(X);
        ...
        void op_num_assign::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
            if (X.isEmpty()) {
                return;
            }
            VarAd y = Y(1);
        ...
        void op_num_assign::setSize(const SetVar &X, const SetVar &Y, VariableMap &data) const {
        }
