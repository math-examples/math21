...
namespace math21 {
    namespace ad {
        ...
        void Function_mat_block_diag_same::cr(const SetVar &X, VarAd x, VarAd y, VarAd dy, SetVar &output, VariableMap &data) const {
            if (X.size() == 0) {
                return;
            }
            ...
            if (X1.isEmpty()) {
                // ..
//                VarAd dx = data.createC(1, "1=d(mat_block_diag_same(x))");
//                if (isSetSize()) {
...
//                data.at(output(1)).setName("d(mat_block_diag_same(x))");
        ...
        void Function_mat_block_diag_same::f(const SetVar &X, SetVar &output, VariableMap &data) {
            if (X.size() == 0) {
                return;
            }
            if (X.size() == 1) {
            ...
            MATH21_ASSERT(X.size() == 2, "not implemented")
            VarAd y = data.createV("mat_block_diag_same(x)");
            if (isSetSize()) {
        ...
        // dot product
        void Function_mat_block_diag_same::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
            if (X.size() == 0) {
                return;
            }
            VarAd y = Y(1);
        ...
        void Function_mat_block_diag_same::setSize(const SetVar &X, const SetVar &Y, VariableMap &data) const {
