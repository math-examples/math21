...
namespace math21 {
    namespace ad {
        ...
        void op_vec_multiply::cr(const SetVar &X, VarAd x, VarAd y, VarAd dy, SetVar &output, VariableMap &data) const {
            SetVar X1;
            X.copyToNox(X1, x);

            op_vec_multiply multiply;
            if (X1.isEmpty()) {
                VarAd dx = data.createC("1=d(op_vec_multiply(x))");
                data.setValue(dx, 1);
                if (isSetSize()) {
        ...
        void op_vec_multiply::f(const SetVar &X, SetVar &output, VariableMap &data) {
            VarAd y = data.createV("op_vec_multiply(x)");
            if (isSetSize()) {
        ...
        // todo: k*x*y
        // dot product
        void op_vec_multiply::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
            VarAd y = Y(1);
        ...
        void op_vec_multiply::setSize(const SetVar &X, const SetVar &Y, VariableMap &data) const {
