...
namespace math21 {
    namespace ad {
        VarAd op_sum::cr_vjp_inner(const SetVar &X, VarAd x, VarAd y, VarAd dy, VariableMap &data) const {
        ...
        // X = {x, axes}
        void op_sum::fv(const SetVar &X, const SetVar &Y, VariableMap &data) const {
            math21_tool_assert(X.size() == 1 || X.size() == 2);
