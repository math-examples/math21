...
//                                                   << ", math21_type_name<T>() = " << math21_type_name<T>()
                                                   << ", math21_type_name<T>() = " << math21_type_name(math21_type_get<T>())
                                                   << "\n");
        }
    ...
    NumB math21_io_load(const char *path, T &x) {
        m21point point = math21_point_read(path);
    ...
    template<typename T>
    NumB math21_io_save(const char *path, const T &x, NumB binary = 1) {
        return math21_point_write(math21_cast_to_point(x), path, binary);
    }
}