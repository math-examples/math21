...
namespace math21 {
    namespace numerical_recipes {
        ...
        NumB GaussJordanElimination::solve(MatR &A_, MatR &B_) {
            LightMatR A(A_);
            LightMatR B(B_);
            MATH21_ASSERT(A.nrows() == A.ncols());
            NumN i, icol, irow, j, k, l, ll, n = A.nrows(), m = B.ncols();
            NumR big, dum, pivinv;
            LightMatN index_c(n), index_r(n), ipiv(n);
            ...
            for (i = 1; i <= n; i++) {
                ...
                ++(ipiv(icol));
                if (irow != icol) {
                    for (l = 1; l <= n; l++) xjswap(A(irow, l), A(icol, l));
                    for (l = 1; l <= m; l++) xjswap(B(irow, l), B(icol, l));
                }
                index_r(i) = irow;
                index_c(i) = icol;
                if (A(icol, icol) == 0.0) {
                    m21warn("Gauss-Jordan elimination: Singular Matrix");
                    if (math21_global_is_debug())A.log("A");
                    return 0;
                }
            ...
            for (l = n; l >= 1; l--) {
                if (index_r(l) != index_c(l))
                    for (k = 1; k <= n; k++)
                        xjswap(A(k, index_r(l)), A(k, index_c(l)));
            }
            return 1;
        }
