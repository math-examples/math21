...
namespace math21 {
    namespace numerical_recipes {
        ...
        public:
            ...
            Fitmrq(const VecR &xx, const VecR &yy, const VecR &ssig, const VecR &aa,
                   ...
                   const NumR TOL = 1.e-3);

            void hold(const NumN i, const NumR val);

            void free(const NumN i);
    ...
    namespace numerical_recipes {
        const VecR &Fitmrq::getOptimal() const {
            return a;
        }

        // data not set zero
        Fitmrq::Fitmrq(const VecR &xx, const VecR &yy, const VecR &ssig, const VecR &aa,
                       void funks(const NumR, const VecR &, NumR &, VecR &),
                       const NumR TOL) : ndat(xx.size()), ma(aa.size()), x(xx), y(yy), sig(ssig),
                                         tol(TOL), funcs(funks), ia(ma), alpha(ma, ma), a(aa), covar(ma, ma) {
            for (NumN i = 1; i <= ma; i++) ia(i) = XJ_TRUE;
        }

        void Fitmrq::hold(const NumN i, const NumR val) {
            ia(i) = XJ_FALSE;
            a(i) = val;
        }

        void Fitmrq::free(const NumN i) { ia(i) = XJ_TRUE; }
...
//                }
        ...
        void Fitmrq::covsrt(MatR &covar) {
            ...
            for (j = ma; j >= 1; j--) {
                if (ia(j)) {
                    for (i = 1; i <= ma; i++) xjswap(covar(i, k), covar(i, j));
                    for (i = 1; i <= ma; i++) xjswap(covar(k, i), covar(j, i));
                    k--;
                }
            }
        }
