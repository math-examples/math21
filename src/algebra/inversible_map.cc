...
namespace math21 {
    ...
    SetN &InversibleMap::getXSet() {
        a.toSet(A);
        MATH21_ASSERT(a.size() == A.size())
        return A;
    }

    SetN &InversibleMap::getYSet() {
        b.toSet(B);
    ...
    // now we check.
    void InversibleMap::add(const SetN &X, const SetN &Y) {
    ...
    void InversibleMap::restrictTo(const SetN &X, InversibleMap &dst) {
        dst.clear();
