...
namespace math21 {
    ...
    // Note: xjmod is different from xjmodulus
    // see xjmod
    template<typename T>
    ...
    template<typename T>
    M21_EXPORT T xjlog_T(T x) { return std::log(x); }
...
// user should check c.
    ...
    template<typename T>
    M21_EXPORT NumR xjround(const T &x) {
        return round(x);
    }

    // round to n decimal places, see round in matlab.
    template<typename T>
    ...
    template<class T>
    M21_EXPORT void xjswap(T &a, T &b) {
        T dum = a;
        a = b;
        b = dum;
    }
    ...
    // d is shape, index from 0
    M21_EXPORT inline void
    xj_index_1d_to_5d(NumN *x1, NumN *x2, NumN *x3, NumN *x4, NumN *x5, NumN y,
