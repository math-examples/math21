...
//#pragma omp parallel for
    ...
    template<typename VecType>
    NumB math21_operator_container_isEqual_num(const VecType &x, NumR k, NumR epsilon = 0) {
        NumN n = x.size();
        MATH21_ASSERT(n >= 1);
        for (NumN i = 1; i <= n; ++i) {
            if (!math21_point_isEqual(x(i), k, epsilon)) {
                return 0;
            }
        }
        return 1;
    ...
    template<typename VecType>
    NumB math21_operator_container_isEqualZero(const VecType &x, NumR epsilon = 0) {
        return math21_operator_container_isEqual_num(x, 0, epsilon);
    }
...
//#pragma omp parallel for
    ...
    // axb = (ax)b, where ax = (0, -a3, a2; a3, 0, -a1; -a2, a1, 0)
    // axb = (a2b3-a3b2, a3b1-a1b3, a1b2-a2b1).t
    template<typename VecType>
...
//#pragma omp parallel for
    ...
    // v is like cdf in order
    template<typename VecType>
    NumN math21_operator_container_cdf_get(const VecType &v, NumR u, NumB includeFirst) {
        NumN n = v.size();
        if (includeFirst) {
            for (NumN i = 1; i <= n; ++i) {
                if (u < v(i)) {
    ...
    // VecType is VecR
    template<typename VecType>
    NumN math21_operator_container_pdf_get(const VecType &v, NumR u) {
        NumN n = v.size();
        // method one
        /*NumR s = 0;
        for (NumN i = 1; i <= n; ++i) {
    ...
    void math21_operator_container_dn_to_d3(const Container<T> &d_x, NumN axis_d2, VecType &d) {
        ...
        math21_operator_container_replace_by_number(d, 1, 0);
    }
    ...
    // fix_3: collapse => [-)[-)|(-]
    template<typename T, template<typename> class Container, typename VecType>
    void math21_operator_container_dn_to_d4_fix_2_or_3(
            const Container<T> &d_x, NumN axis_d2, NumN axis_d3, VecType &d, NumB fix2) {
        d.setSize(4);
        NumN p1, p2;
        p1 = axis_d2;
        p2 = axis_d3;
        ...
        if (fix2) {
            d(2) = d_x(p1);
            d(3) = math21_operator_container_multiply_some(d_x, p2 - p1, p1);
        } else { // fix 3
            d(2) = math21_operator_container_multiply_some(d_x, p2 - p1, p1 - 1);
            d(3) = d_x(p2);
        }
        d(4) = math21_operator_container_multiply_some(d_x, d_x.size() - p2, p2);
        math21_operator_container_replace_by_number(d, 1, 0);
    }

    template<typename T, template<typename> class Container, typename VecType>
    void math21_operator_container_dn_to_d4_fix_2or3(
            const Container<T> &d_x, NumN from, NumN to, VecType &d) {
        NumN p1, p2;
        p1 = xjmin(from, to);
        p2 = xjmax(from, to);
        math21_operator_container_dn_to_d4_fix_2_or_3(d_x, p1, p2, d, from<to);
    }

    void math21_operator_vector_mask_to_index(const VecN &mask, VecN &index);
}