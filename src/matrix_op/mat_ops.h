...
namespace math21 {
    ...
    void math21_operator_swap_rows(Tensor<T> &A, NumN i, NumN j) {
        ...
        NumN k;
        for (k = 1; k <= n; ++k) {
            xjswap(A.operator()(i, k), A.operator()(j, k));
        }
    }
    ...
    void math21_operator_swap_cols(Tensor<T> &A, NumN i, NumN j) {
        ...
        NumN k;
        for (k = 1; k <= n; ++k) {
            xjswap(A.operator()(k, i), A.operator()(k, j));
        }
    }
...
// A is Mat, b is Vec, A <- A + v
    ...
    // see math21_op_submatrix_get
    template<typename T>
    void math21_operator_matrix_submatrix(
            const Tensor<T> &A, Tensor<T> &B, const VecN &row_indexes, const VecN &col_indexes) {
    ...
    // see math21_matlab_diag
    template<typename T>
    void math21_operator_matrix_diag_do(const Tensor<T> &A, Tensor<T> &B, NumZ n=0) {
        if (n == 0) {
            if (A.isVectorInMath()) {
    ...
    void math21_operator_matrix_trans(Tensor<T> &A) {
        ...
        for (i = 1; i <= n; i++) {
            for (j = i + 1; j <= m; j++) {
                xjswap(A.operator()(i, j), A.operator()(j, i));
            }
        }
    }
    ...
    // matrix norm
    // n = 1: max(sum(abs(X), 1))
    // n = inf: max(sum(abs(X), 2))
    // n = 2: max(svd(X))
    // Frobenius norm: vector_norm(X, 2)
    template<typename T>
    NumR _math21_operator_mat_norm(const Tensor <T> &x, NumN type) {
        NumR value = 0;
        ...
        if (type == 1 || type == 2) {
            ...
            math21_op_sum(x_abs, y, axes);
            value = math21_op_vector_max(y);
        } else if (type == 3) {
            VecR y;
            math21_operator_svd_singular_values(x, y);
            value = math21_op_vector_max(y);
        } else {
            MATH21_ASSERT(0);
        }
    ...
    template<typename T>
    NumR math21_operator_mat_norm(const Tensor <T> &x, NumN n) {
        if (n == 1) {
            return _math21_operator_mat_norm(x, 1);
        } else if (n == 2) {
            return _math21_operator_mat_norm(x, 3);
        } else {
    ...
    template<typename T>
    NumR math21_operator_mat_norm(const Tensor <T> &x, const std::string &name) {
        if (math21_string_is_equal(name, "inf")) {
            return _math21_operator_mat_norm(x, 2);
        } else if (math21_string_is_equal(name, "frob")) {
            return math21_op_vector_norm(x, 2);
        } else {
            MATH21_ASSERT(0);
        }
        return 0;
    ...
    template<typename T>
    NumB math21_operator_mat_is_symmetric(const Tensor<T> &x) {
        if (x.nrows() != x.ncols())return 0;
        NumN n = x.nrows();
        for (NumN i = 1; i <= n; ++i) {
            for (NumN j = i + 1; j <= n; ++j) {
                if (!math21_operator_num_isEqual(x(i, j), x(j, i), MATH21_EPSILON))return 0;
            }
        }
        return 1;
    }
