...
namespace math21 {
    ...
    template<typename T>
    void math21_operator_vec_to_set(const Tensor<T> &x, Set_<T> &S) {
        NumN n = x.size();
        S.clear();
        for (NumN i = 1; i <= n; i++) {
            S.add(x(i));
    ...
    template<typename T>
    void math21_operator_set_to_vec(const Set_<T> &S, Tensor<T> &x) {
        NumN n = S.size();
        x.setSize(n);
        for (NumN i = 1; i <= n; i++) {
            x(i) = S(i);
    ...
    void math21_operator_vector_find_subvector(const Tensor<T> &v, const Tensor<T> &s, VecN &where,
        ...
        if (which_.isEmpty()) {
            ...
            math21_operator_container_assign(index, where);
            return;
        }
        VecN which(which_.size());
        math21_operator_container_pos_correct(which_, which, index.size());
        SetN S;
        math21_convert_container_to_set(which, S);
        S.sort();
        NumN offset = 0;
        if (!S.isEmpty() && S(1) == 0) offset = 1;
        math21_convert_set_to_container(S, which, 0, offset);
        where.setSize(which.size());
        math21_operator_container_set_by_map(index, where, which);
    }

    template<typename T>
    void math21_operator_vector_replace(const Tensor<T> &x, Tensor<T> &y, const Tensor<T> &s, const Tensor<T> &t,
                                        const VecZ &which = VecZ()) {
        VecN where;
        math21_operator_vector_find_subvector(x, s, where, which);
