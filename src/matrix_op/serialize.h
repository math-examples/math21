...
//                m = m_c;
            ...
            MATH21_ASSERT(m.isSameSize(d));
        }
    }


    template<typename T>
    void math21_io_serialize(std::ostream &out, const LightMatrix <T> &m, SerializeNumInterface &sn) {
        Tensor<T> x;
        m.copyTo(x);
        math21_io_serialize(out, x, sn);
    }

    template<typename T>
    void math21_io_deserialize(std::istream &in, LightMatrix <T> &m, DeserializeNumInterface &sn) {
        Tensor<T> x;
        math21_io_deserialize(in, x, sn);
        m.copyFrom(x);
    }
