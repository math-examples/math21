...
namespace math21 {
    ...
    // todo: remove duplicate code, see ad_gmm_log_likelihood
    void math21_operator_gmm_unpack_params(const VecR &params,
                                           VecR &log_proportions, MatR &means, TenR &covs_sqrt,
                                           NumN n_component, NumN n_feature, NumB isECorder) {
        VecR tmp;
        math21_operator_share_to_vector(params, log_proportions, n_component);
        math21_op_logsumexp(log_proportions, tmp, VecN());
        math21_op_subtract_onto_1(log_proportions, tmp);

        NumN offset_mean, offset_cov;
        if (isECorder) {
            offset_mean = n_component;
            offset_cov = n_component + n_component * n_feature;
        } else {
            offset_cov = n_component;
            offset_mean = n_component + n_component * n_feature * n_feature;
        }
        math21_operator_share_to_matrix(params, means, n_component, n_feature, offset_mean);
        math21_operator_share_to_tensor3d(params, covs_sqrt, n_component, n_feature, n_feature, offset_cov);
    }
}